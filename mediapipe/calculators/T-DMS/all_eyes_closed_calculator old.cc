...
class AllEyesClosedCalculator : public CalculatorBase {
private:
	bool verbose_; // Verbose logging flag
	float ear_threshold_; // Eye Aspect Ratio threshold for determining if the eye is closed
	float closed_duration_s_threshold_; // Duration in seconds to consider the eye closed
    std::chrono::steady_clock::time_point closed_start_time_;
...
public:
	...
	absl::Status Open(CalculatorContext* cc) override {
		...
		closed_duration_s_threshold_ = opts.closed_duration_s_threshold();
	...
	absl::Status Process(CalculatorContext* cc) override {
        ...
        if (all_closed) {
            if (!timer_started_) {
                closed_start_time_ = now;
                timer_started_ = true;
            }
            float closed_duration = std::chrono::duration<float>(now - closed_start_time_).count();
            if (closed_duration >= closed_duration_s_threshold_) {
                result = true;
            }
            if (verbose_) {
                LOG(INFO) << "Eyes closed for " << closed_duration << "s (threshold: " << closed_duration_s_threshold_ << ")";
            }
        } else {
            timer_started_ = false;
        }
        ...
        if (verbose_) {
            LOG(INFO) << "All closed: " << all_closed << ", Result: " << result;
        }
